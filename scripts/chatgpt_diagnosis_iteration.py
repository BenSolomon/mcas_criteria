...
def diagnosis_query(query, gpt_model = "gpt-3.5-turbo-1106"):
    try:
...
def openai_wrapper(i, criteria, symptoms, gpt_version='gpt-3.5-turbo-1106', output_query=False):
    query = build_query(symptoms)
    json_output = diagnosis_query(query, gpt_version)
    output = {'i':i, 'criteria':criteria, 'symptoms':symptoms.split(",")}
    if output_query:
...
def chatgpt_pipeline(iteration_path, output_dir, batch_size = 1000, gpt_version = "gpt-3.5-turbo-1106"):
    ...
    for i in batches:
        ...
        # Map openai_wraper to criteria iteration data contained in each row
        output = df \
            .apply(lambda row: openai_wrapper(
