...
df_entropy_bootstrap <- df %>% 
  ...
  mutate(boot = map(data, function(x){
    x <- deframe(x)
    x <- entropart::EntropyCI(Shannon, Simulations=5000, Ns = x, q=1, Correction = "None")
    x
  })) %>% 
...
df_entropy_bootstrap %>% 
  ...
  geom_point(size = 0.5)+
  geom_errorbar(aes(ymin = `2.5%`, ymax = `97.5%`), width = 0.4, stat = "identity")+
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x="", y = "Shannon diversity")
```
...
plt_div <- broom::tidy(div_diag) %>% 
  mutate(names = factor(names, levels = criteria_order)) %>% 
  ggplot(aes(x=names, y = x))+
  geom_bar(stat = "identity", color = "black") +
  ggpubr::stat_pvalue_manual(
    df_entropy_diff_pvales %>%
