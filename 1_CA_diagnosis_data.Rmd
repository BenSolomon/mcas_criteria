...
mcas_plt <- disease_plot("MAST CELL ACTIVATION") + ggtitle("Mast cell activation syndrome") + scale_x_continuous(limits = c(2005,2021), n.breaks = length(unique(df$year)))
...
plt_icd_trend <- df %>% 
  ...
  scale_color_brewer(palette = "Set1", labels = c("Mast cell activation\nsyndrome", "Mastocytosis"))
plt_icd_trend
```
```{r}
