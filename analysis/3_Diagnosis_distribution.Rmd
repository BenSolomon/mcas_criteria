...
rank_table <-
  multi_diagnosis_rank_table(
    search_pattern = "T78\\.2 |D47\\.02 |D89\\.40 |D89\\.41 |D89\\.42 |D89\\.43 |D89\\.44 |D89\\.49 |D89\\.4 ",
    df_gpt3.5_icd,
    df_gpt4.0_icd,
    df_claude3_haiku_t1.0_icd,
    df_claude3_opus_t1.0_icd,
...
df_diversity_n_correlation %>% 
  ...
  geom_smooth(method = "lm", group = 1, se = F)+
  ggpubr::stat_cor(method = "pearson", geom = "label")+
...
```{r}
vroom::vroom(here("data/criteria_query_iterations.csv")) %>% 
  mutate(symptoms = str_split(symptoms, pattern = ", ")) %>% 
  mutate(symptoms = map(symptoms, sort)) %>% 
  mutate(symptoms = map_chr(symptoms, ~paste(., collapse = ", "))) %>% 
  count(criteria, symptoms, sort = T) %>% 
  filter(grepl("alternative", criteria))
```
...
```{r, fig.width=4, fig.height=3.5}
# Limit P-values displayed
df_precision <- read_csv(here("data/diversity_analysis/compiled_icd_diagnosis_precision.csv")) %>% 
  precision_dist_to_sim() %>% 
  format_criteria() %>% 
  format_models() %>%
  filter(model != "Gemini 1.5 Flash")

ggplot(df_precision, aes(x = criteria, y = mean))+
  theme_bw()+
  theme(axis.text.x = element_text(angle= 45, hjust = 1))+
  labs(x="", y = "Average Bray-Curtis Similarity")+
  mcas_only_pwc(df_precision, variable = "mean") +
  labs(color=NULL)+
  scale_color_brewer(palette = "Dark2") +
  plot_selector("points")
...
### Version 3
```{r, fig.width=7.5, fig.height=8.6, message = F}
n_diagnoses_bar <- 10
n_diagnoses_abundance <- 50
n_diagnoses_cumulative <- 50
...
plt_precision <- ggplot(df_precision, aes(x = criteria, y = mean))+
  theme_bw()+
  theme(axis.text.x = element_text(angle= 45, hjust = 1))+
...
unlabeled_plt <- plot_grid(
  ...
  ncol = 1,
  rel_heights = c(0.01,1.2, 0.032, 0.64)
)  

full_plt <-
  cowplot::ggdraw(unlabeled_plt) + cowplot::draw_plot_label(
    LETTERS[1:10],
    x = c(0, 0.49, 0, 0.49, 0, 0.49, 0, 0.34, 0.67, 0.83),
    y = c(1, 1, 0.820, 0.820, 0.64, 0.64, 0.36, 0.36, 0.36, 0.36)
  )
...
```{r, eval=F}
ggsave(plot=full_plt,filename=here("figures/3_diagnosis_diversity.pdf"), width = 7.5, height = 8.6)
```
...
multi_diagnosis_rank_table(search_pattern = "T78\\.2 |D47\\.02 |D89\\.40 |D89\\.41 |D89\\.42 |D89\\.43 |D89\\.44 |D89\\.49 |D89\\.4 ",
                                         df_gpt3.5_icd, df_gpt4.0_icd, df_claude3_haiku_t1.0_icd, df_claude3_opus_t1.0_icd, df_gemini1.0_pro_t1.0_icd, df_gemini1.5_pro_t1.0_icd) %>% 
  flextable() %>% 
  width(width = 2) %>% 
  fontsize(size = 9) %>% 
