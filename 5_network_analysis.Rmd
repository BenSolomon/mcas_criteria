...
```
### Replacing node colors in facets manually


- Create data frame with diagnosis and rank for each criteria
- Create a column for color based on rank
...
```{r}
# Create base plot
plt <- g %>% activate(nodes) %>% 
  ggraph(layout = 'fr') +
  geom_edge_link(alpha = 0.5) +
  geom_node_point(size = 0.5) +
  geom_node_text(aes(label = name))+ # Creating a label layer creates a target to join df_diagnoses to. 
  facet_wrap(~criteria) +
  theme_void() +
  theme(panel.border = element_rect(color = "black", fill = NA, size = 1)) +
```
...
colors <- plt_build$data[[3]] %>% 
  left_join(facet_key, by = "PANEL") %>% # Join facet criteria name
  left_join(df_diagnoses, by = c("label" = "name", "criteria")) %>% # Join diagnosis info including colors
  mutate(color = replace_na(color, "black")) %>% # Make all nodes not ranked in a criteria black (otherwise dropped)
  pull(color)

plt_build$data[[2]]$colour <- colors # Assign colors to a color column. The order of rows in each layer data frame is the same
plt_build$data[[3]] <- NULL # Remove geom_text layer
