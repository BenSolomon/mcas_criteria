...
labeller_fun <- function(x){
  list(criteria = c("mcas_consortium", "mcas_alternative", "eular_acr_sle", "slicc_sle")) 
}
...
plt <- ggraph(g_top_codiags_rank, layout = 'fr') +
  ...
  facet_wrap(~criteria, labeller = labeller_fun) +
  theme_void() +
  theme(panel.border = element_rect(color = "black", fill = NA, size = 1)) +
...
```
```{r}
ggsave(here("figures/3_network.pdf"), plot=plt_figure, width = 3.5, height = 6)
```
